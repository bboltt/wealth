...
def calculate_client_features(df, metric, periods):
    ...
    for months in periods:
        # Rolling sum for previous months within the same client, excluding the current month
        # Use rowsBetween to specify the exact number of preceding rows based on months
        rolling_avg = avg(metric).over(windowSpec.rowsBetween(-months, -1))
        
        # Calculate difference and percentage difference
        df = df.withColumn(f"{metric}_diff_from_m{months}", col(metric) - rolling_avg)
        df = df.withColumn(f"{metric}_sum_pct_diff_from_m{months}",
                           (col(metric) - rolling_avg) / when(rolling_avg != 0, rolling_avg).otherwise(1))
    return df
